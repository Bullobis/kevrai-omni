...
- **应用设置**（目录、主题、硬件加速、Token 等）保存在用户数据目录的 `settings.json`。
- **模型收藏、最近使用、命令面板历史**保存在渲染层本地存储（localStorage）中。

两种备份方式：

1. **一键导出（推荐）**：打开 **设置中心 → 数据管理**，选择导出，把备份文件存到安全位置。
2. **手动备份**：按 `Ctrl/⌘+K` 打开命令面板，执行「打开数据目录」，把整个 `KevraiOmni` 文件夹（至少 `settings.json` 与 `models/`、`engines/`）拷出来。用户数据目录位置见 Q24。

迁移到新机器时，把备份放回到目标机器的相同用户数据目录即可，启动后自动识别。

---

### Q27. 导入备份后，我的收藏和最近使用去哪了？

收藏与最近使用是按模型 ID 记录的。导入后：

- 如果你导入的是同一套模型市场目录，收藏和最近使用会原样恢复。
- 如果你只导入了配置备份、但没有把对应的模型文件一起拷过来，收藏的模型可能显示为「未下载 / 不可用」——这是正常的，把模型文件补上即可，收藏记录不会丢。
- 想确认收藏是否回来：在模型市场工具栏的「收藏 / 最近使用」筛选里切到「★ 仅收藏」查看。

---

### Q28. 导入备份会覆盖我现有的数据吗？

会。导入备份是以备份文件中的内容为准进行恢复：

- 设置项会被备份里的值替换（目录、主题、硬件加速、Token 等以备份为准）。
- 收藏、最近使用等个性化列表会被备份中的列表替换。

**导入前建议先在「数据管理」里导出一份当前状态**，万一覆盖错了可以再导回去。模型文件本身（`models/`、`engines/` 下的实际权重）一般不会被配置导入操作删除。
